...
local topHeight=SCALEY(12)
local edgeWidth=SCALEX(10)
...
function ZyButton:new(picNor, picDown, picDis, title, fontName, fontSize)
    ...
    elseif picNor then
        ...
        if title then
            ...
            spriteDown:setPosition(CCPoint(0, SCALEY(-1)))
        else
...
function ZyMessageBoxEx:initMessageBox()
    ...
    if self._tableStyle[MB_STYLE_MESSAGE] ~= nil then
        ...
        elseif self._tableStyle[MB_STYLE_RBUTTON] ~= nil and self._tableStyle[MB_STYLE_LBUTTON] ~= nil then
            ...
            offsetX = boxSize.width*0.9-button:getContentSize().width-edgeWidth+SCALEX(5)
            button:setPosition(CCPoint(offsetX, topHeight))
            messageBox:addChild(button, 0, 0)
            gClassPool[item] = self
            button, item = UIHelper.Button(P(ButtonNor), P(ButtonClk), actionMessageboxRightButton,
            self._tableStyle[MB_STYLE_RBUTTON], kCCTextAlignmentCenter, FONT_SMM_SIZE);
            offsetX =edgeWidth -SCALEX(5)+boxSize.width*0.1
            button:setPosition(CCPoint(offsetX, topHeight));
            messageBox:addChild(button, 0, 0)
            gClassPool[item] = self
        end
    ...
    if self._tableStyle[MB_STYLE_MODIFY] ~= nil then
        local offsetX =edgeWidth+SCALEX(4)
        local offsetY =boxSize.height/2+SCALEY(6)
    ...
    --  offsetY = offsetY + label:getContentSize().height/2
        local size = CCSize(boxSize.width/3, SCALEY(22))
        local edit = CScutEdit:new()
        edit:init(false, false)

        offsetX = messageBox:getPosition().x + edgeWidth+label:getContentSize().width+SCALEY(6)
        offsetY =pWinSize.height-messageBox:getPosition().y-boxSize.height/2-SCALEY(6)-size.height+label:getContentSize().height
    ...
    if self._tableStyle[MB_STYLE_RENAME] ~= nil then
        local offsetX = nil
        local offsetY =boxSize.height*0.5     --+SCALEY(6)
        local nameW = 0
        ...
        offsetY = offsetY+oldLabel:getContentSize().height+SCALEY(5)
        ...
        offsetY = parentSize.height/2--size.height/2+oldLabel:getContentSize().height-SCALEY(5)
        edit:setRect(CCRect(offsetX, offsetY, size.width, size.height))
        self._edit = edit
    end
...
function showRank()
	...
	local csvConfig = loadCsvFile("resource\aa.Csv");
...
function createUIBg(titleImagePath,titleStr,textColor,closeBtnActionPath, touming)
    ...
    if closeBtnActionPath ~= nil then
        ...
        closeBtn:setPosition(CCPoint(layer:getContentSize().width - closeBtn:getContentSize().width,layer:getContentSize().height - closeBtn:getContentSize().height-SCALEY(3)))
    end
    ...
    if titleStr then
        ...
        titleStrLabel:setPosition(CCPoint(pWinSize.width*0.5,pWinSize.height-titleStrLabel:getContentSize().height-SCALEY(15)))
...
function submit()
    ...
    submitLayer:setContentSize(CCSize(SCALEX(240),SCALEY(160)));
    ...
    sprite:setScaleX(SCALEX(240)/sprite:getContentSize().width);
    sprite:setScaleY(SCALEY(160)/sprite:getContentSize().height);
    ...
    titleName1:setPosition(CCPoint(SCALEX(-100),
        startY+titleName1:getContentSize().height - SCALEY(30)))
    submitLayer:addChild(titleName1)
    titleName1:setColor(ccc3(0,0,0))

    local txt_x=titleName1:getPositionX()+SCALEX(8)+titleName1:getContentSize().width
    local txt_ww=xx+ww-txt_x-SCALEX(44)
    
    local bgEmCCPointy1= CCSprite:create(P(image_list_txt))
  
    mNameEdit =  CCEditBox:create(CCSize(SCALEX(120),bgEmCCPointy1:getContentSize().height), CCScale9Sprite:create(P(image_list_txt)))
    mNameEdit:setPosition(CCPoint(titleName1:getPositionX()+ titleName1:getContentSize().width +  SCALEX(60) ,titleName1:getPositionY()+SCALEY(5)))
    ...
    titleName:setPosition(CCPoint(titleName1:getPositionX(),titleName1:getPositionY()+txt_h+SCALEY(10)))
    submitLayer:addChild(titleName)

    mScoreEdit = CCEditBox:create(CCSize(SCALEX(120),bgEmCCPointy1:getContentSize().height), CCScale9Sprite:create(P(image_list_txt)))
    mScoreEdit:setFontColor(ccc3(0,0,0))
    mScoreEdit:setPosition(CCPoint(titleName:getPositionX() + titleName:getContentSize().width + SCALEX(60) ,titleName:getPositionY()+SCALEY(5)))
    submitLayer:addChild(mScoreEdit)
    mScoreEdit:setVisible(true)
    
    local button2 = ZyButton:new("button/button_1011.png", "button/button_1012.png",nil,IDS_OK)
    button2:setPosition(PT(SCALEX(-30) -button2:getContentSize().width,SCALEY(-50)));
    ...
    button3:setPosition(PT(SCALEX(30) ,SCALEY(-50)));
...
function init()
    ...
    button:setPosition(PT(pWinSize.width - button:getContentSize().width - SCALEX(10), SCALEY(10)));
	...
	labelIds1:setPosition(PT(SCALEX(20) + labelIds1:getContentSize().width/2 , pWinSize.height - SCALEY(18)));
...
function netCallback(pZyScene, lpExternalData, isTcp)
    ...
    if actionID==1001 then
		...
		if labelIds4 == nil  then 
			...
			labelIds4:setPosition(PT(labelIds3:getPositionX() , labelIds3:getPositionY() - SCALEY(15)));
        ...
        if table then
		    if bgLayer == nil then 
				...
				closeBtn:setPosition(PT(bgLayer:getContentSize().width-closeBtn:getContentSize().width - SCALEX(15),bgLayer:getContentSize().height-closeBtn:getContentSize().height - SCALEY(5)));
...
function showLayout(data)
   ...
   for i,v in ipairs(table) do
        ...
        textlabel:setPosition(PT(cursor+simpleW*temp/2,listBgStartY+SCALEY(2)))
...
function update(mtable)
    ...
    for i, v in ipairs(mtable) do
        ...
        bgLayer:setPosition(PT(-SCALEX(50),layer:getContentSize().height-i*bgEmpty:getContentSize().height))
